...
export const hash = async (password: string, s?: string) => {
  const salt = s ? decodeBase64(s) : crypto.getRandomValues(new Uint8Array(16));
  ...
  return `${encodeBase64(salt)}$$${encodeBase64(hashed)}`;
};
...
export const verify = async (password: string, stored: string) => {
  ...
  return timingSafeEqual(decodeBase64(hashStored), decodeBase64(passIn));
};

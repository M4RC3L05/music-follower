...
describe("upsertMany()", () => {
  ...
  describe("collection type", () => {
    ...
    it("should update a release if one already exists", () => {
      ...
      expect(releasesTable.all(sql`select * from $${releasesTable.lit("table")}`)).toHaveLength(1);
    });
  });
  ...
  describe("track type", () => {
    ...
    it("should create a release if one does not exist", () => {
      ...
      expect(releasesTable.all(sql`select * from $${releasesTable.lit("table")}`)).toHaveLength(2);
    });
